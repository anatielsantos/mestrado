...
def load_images(path_src, path_mask):
    ...
    return np.float32(np.expand_dims(np.concatenate(src), axis=-1)), np.expand_dims(np.concatenate(tar), axis=-1)
...
def train(path_weights, src_images_train, tar_images_train):    
    ...
    # train model
    checkpoint = ModelCheckpoint(path_weights+'gan_test2_exp2_1_150epc_best.hdf5', monitor='dice', verbose=1, save_best_only=True,save_weights_only=True, mode='max')
    ...
    model.save(path_weights+'gan_test2_exp2_1_150epc_last.hdf5')
    ...
    hist_json_file = path_json+'gan_test2_exp2_1_history_150epc.json' 
    ...
    # save plot to file
    plt.savefig(path_plot+'gan_test2_exp2_1_plot_150epc.png')
    # plt.show()
