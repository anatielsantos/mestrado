...
class Pix2Pix(tf.keras.Model):
    ...
    def build_discriminator(self):
        ...
        inp = tf.keras.layers.Input(
            shape=[self.image_height, self.image_width, self.input_channels], name='input_image')
        tar = tf.keras.layers.Input(
            shape=[self.image_height, self.image_width, self.output_channels], name='target_image')
    ...
    def save(self, filepath, overwrite=True, include_optimizer=True, save_format=None, signatures=None, options=None):
        self.generator.save(filepath, overwrite,
